...
public class SmsUtil {
    ...
    public static String sendSms(String phone,String smsTemp,String code) throws ClientException {
        ...
        map.put("code", code);
        request.setTemplateParam(JacksonUtil.obj2Json(map));
...
//        request.setOutId("123");
        ...
        String status = "";
        String codeStatus = sendSmsResponse.getCode();
        String message = sendSmsResponse.getMessage();
        if(codeStatus != null && codeStatus.equals("OK")) {
        	status = "success";
        	} else if (codeStatus.equals("isv.BUSINESS_LIMIT_CONTROL")){
        		if (message.equals("触发分钟级流控Permits:1")) {
        			status = "fail";
        		} else if (message.equals("触发小时级流控Permits:5")) {
        			status = "fail";
        		} else {
        		}
        	}
